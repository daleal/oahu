...
Lightweight Rails 6 template built with `Docker` and `docker-compose` to **_plug and play_** with a final image size of just `~301MB`.
...
The image generated by `docker-compose build` with the original repo has a size of just `~301MB`, and contains a standard Rails 6 app using `postgresql` as the database, `sass` as the stylesheet pre-processor and `puma` as the webserver.
